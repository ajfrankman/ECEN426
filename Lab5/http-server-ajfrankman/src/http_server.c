...
int http_server_parse_arguments(int argc, char *argv[], Config *config) {
    ...
    if (portSet == 0) {
        log_info("Setting default port: \"8084\".");
...
int http_server_send_response(int socket, Response response) {
    ...
    // Send Status Line
    statusLine = malloc(11 + strlen(response.status));
    ...
    header = malloc(strlen(response.headers[0]->name) + strlen(response.headers[0]->value) + 6);
...
int http_server_process_request(Request request, char *relative_path, Response *response) {
    ...
    response->headers[0]->name = malloc(15);
