...
int tcp_client_parse_arguments(int argc, char *argv[], Config *config) {
    ...
    if ((argc - optind) != SERVER_ARGS) {
        log_error("Invalid argument amount\n\n");
        log_info("%s\n\n", usagePattern);
        return 1;
    }
...
int tcp_client_send_request(int sockfd, char *action, char *message) {
    char *fullMessage; // String to append full message to.

    int actionBytes = strlen(action);
    int messageBytes = strlen(message);

    fullMessage = (char *)malloc((actionBytes + messageBytes + 10) * sizeof(char));

    printIfVerbose("Building message");
    sprintf(fullMessage, "%s %ld %s", action, strlen(message), message);
    printIfVerbose("Sending message: ");
    printIfVerbose(fullMessage);
...
Return value:
    Returns a fullMessageLength on success, and 0 if a full message doesn't exist.
*/
int checkForMessage(char *message, int totalBytes) {
    // Check for number in first element. isdigit
    if (isdigit(message[0])) {
        // Read the number in.
        int length = atoi(message);
        int headerLength = getHeaderLength(message);
        // Check if there are enough characters after.
        if (totalBytes >= (length + headerLength)) {
...
int tcp_client_receive_response(int sockfd, int (*handle_response)(char *)) {
    ...
    while (true) {
        ...
        while ((movePointer = checkForMessage(mainBuf, totalBytesRecevied)) > 0) {
            ...
            char *handleString;
            handleString = (char *)malloc(movePointer * sizeof(char));
            int headerLength = getHeaderLength(mainBuf);
...
int tcp_client_get_line(FILE *fd, char **action, char **message) {
    const char delim[] = " ";   // To split action and message.
    const char delim2[] = "\n"; // To split end line and message.
    ssize_t read;
    char *line = NULL;
    size_t len = 0;
    ...
    *action = (char *)malloc((read + 1) * sizeof(char));
    printIfVerbose("Parsing Action.");
    *action = strtok(line, delim);

    *message = (char *)malloc((read + 1) * sizeof(char));
